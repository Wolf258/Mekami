...
func TestLoad(t *testing.T) {
	...
	t.Run("partial_keeps_unset_defaults", func(t *testing.T) {
		dir := t.TempDir()
	...
	t.Run("unknown_field_errors", func(t *testing.T) {
		dir := t.TempDir()
		path := filepath.Join(dir, "config.json")
	...
	t.Run("bad_version_errors", func(t *testing.T) {
		dir := t.TempDir()
		path := filepath.Join(dir, "config.json")
...
func TestShouldLog(t *testing.T) {
	cases := []struct {
		...
		{"empty_log_info", "", "info", true},
		{"empty_log_debug", "", "debug", false},
		{"info_log_info", "info", "info", true},
		{"info_log_debug", "info", "debug", false},
		{"debug_log_info", "debug", "info", true},
		{"debug_log_debug", "debug", "debug", true},
		{"quiet_log_info", "quiet", "info", false},
		{"quiet_log_error", "quiet", "error", false},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			if got := (WatchConfig{Log: tc.log}).ShouldLog(tc.level); got != tc.want {
