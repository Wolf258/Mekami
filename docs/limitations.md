...
- **No body text in the index.** Mekami only indexes symbol names and reference edges. For substring search inside function bodies, comments, log strings, or TODOs, use `mekami find-text` (server-side regex over the source tree) or your editor's read tool.
